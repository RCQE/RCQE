...
def read_arguments():
	...
	parser.add_argument("--output_dir", default="./mcc_model", type=str, required=False,
...
def main(args):
	...
	# read model --------------------------------------------------------------
	model_config = T5Config.from_pretrained("/data/hzh22/CodeBERT/CodeReviewer/model/CodeReviewer")
	plm = T5ForConditionalGeneration.from_pretrained("/data/hzh22/CodeBERT/CodeReviewer/model/CodeReviewer", config=model_config)
	tokenizer = RobertaTokenizer.from_pretrained("/data/hzh22/CodeBERT/CodeReviewer/model/CodeReviewer")
	WrapperClass = T5TokenizerWrapper

	# define template
	promptTemplate = PrefixTuningTemplate(model=plm, tokenizer=tokenizer,
                text='Code Diff: {"placeholder":"text_a"} The New Diff Change for Review Comment Asked is {"mask"} ', using_decoder_past_key_values=False, num_token=200)
