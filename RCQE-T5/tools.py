...
def read_prompt_examples(file_type):
    ...
    file_name = '../data2/ref-{}.jsonl'.format(file_type)
    with open(file_name, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            line = line.strip()
            js = json.loads(line)
            old = js["old_hunk"].split("\n")[1:] 
            new = js["hunk"].split("\n")[1:] 
            old_diff = "".join(old)
            new_diff = "".join(new)
            if 'idx' not in js:
            ...
            result = "Old Diff: " + old_diff + ", New Diff: " + new_diff
            comment = js["comment"]
            example = InputExample(guid=idx, text_a=result, text_b=comment, tgt_text=tgt_txt)
