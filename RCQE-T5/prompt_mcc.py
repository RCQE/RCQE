...
import bleu
from tools import read_prompt_examples, get_elapse_time
...
def read_arguments():
	...
	parser.add_argument("--output_dir", default="./mcc_model_without_comment", type=str, required=False,
...
def main(args):
	...
	# read model --------------------------------------------------------------
	model_config = T5Config.from_pretrained("/data/hzh22/pt4code/PT4Code/defect/prompt/model/codet5/codet5_small")
	plm = T5ForConditionalGeneration.from_pretrained("/data/hzh22/pt4code/PT4Code/defect/prompt/model/codet5/codet5_small", config=model_config)
	tokenizer = RobertaTokenizer.from_pretrained("/data/hzh22/pt4code/PT4Code/defect/prompt/model/codet5/codet5_small")
	WrapperClass = T5TokenizerWrapper

	# define template
	promptTemplate = PrefixTuningTemplate(model=plm, tokenizer=tokenizer,
                text='Code Diff: {"placeholder":"text_a"} Review Comment: {"placeholder":"text_b"} The New Diff Change for Review Comment Asked is {"mask"} ', using_decoder_past_key_values=False, num_token=200)
	...
	if args.do_train:
		...
		train_examples = read_prompt_examples('train')
		# train_examples = random.sample(train_examples, 16)
		# take an example
		...
		for epoch in range(args.num_train_epochs):
			...
			if args.do_eval:
				...
				if eval_dataloader is None:
					# Prepare training data loader
					eval_examples = read_prompt_examples('valid')
				...
				this_bleu, dev_dataloader = calculate_bleu('valid', args, tokenizer, device, model, promptTemplate, WrapperClass, is_test=False, dev_dataloader=dev_dataloader, best_bleu=best_bleu)
	...
	# use test file ( if exist) to calculate bleu
	if args.do_test:
		calculate_bleu('test', args, tokenizer, device, model, promptTemplate, WrapperClass, output_file_name='0', is_test=True)
