...
from utils import RefineDataset, SimpleRefineDataset,SimpleElsDataset,ElsDataset
...
def get_loader(data_file, args, tokenizer, pool, eval=False):
    ...
    if args.raw_input:
        dataset = SimpleElsDataset(tokenizer, pool, args, data_file)
    else:
        dataset = ElsDataset(tokenizer, pool, args, data_file)
...
def eval_bleu_epoch(args, eval_dataloader, model, tokenizer):
    ...
    for i in range(len(golds)):
        pred_nls[i], golds[i] = ElsDataset.process_pred_gold(pred_nls[i], golds[i])
    ...
    for epoch in range(1, args.train_epochs + 1):
        ...
        for step, examples in enumerate(train_dataloader, 1):
            ...
            if global_step == args.train_steps and args.global_rank == 0:
                ...
                output_dir = os.path.join(args.output_dir, "checkpoints-last" + "-" + "2.0")
