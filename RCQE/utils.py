    ...
    def tokenize(self, item):
        ...
        for label, line in zip(labels, difflines):
            if label == 1:
                ...
                inputstr += "<keep>" + line
                difflines = example["old_hunk"].split("\n")[1:]        # remove start @@
        newdiff = example["hunk"]
        newdifflines = newdiff.split("\n")[1:]        # remove start @@
        newdifflines = [line for line in newdifflines if len(line.strip()) > 0]
        newlabels = [f(line[0]) for line in newdifflines]
        newdifflines = [line[1:].strip() for line in newdifflines]
        newinputstr = ""
        for label, line in zip(newlabels, newdifflines):
            if label == 1:
                newinputstr += "<add>" + line
            elif label == 0:
                newinputstr += "<del>" + line
            else:
                newinputstr += "<keep>" + line
                newdifflines = example["hunk"].split("\n")[1:]
        # oldlines = example["old_hunk"].split("@@")[2].strip().split("\n")
        # newlines = example["hunk"].split("@@")[2].strip().split("\n")
        # oldlines = [line[1:].strip() for line in oldlines]
        # newlines = [line[1:].strip() for line in newlines]
        # oldlines = "\n".join(oldlines)
        # newlines = "\n".join(newlines)
        ...
        # newlines = "<add>" + newlines.replace("\n", "<add>")
        comment = example["comment"]
        srcids = [tokenizer.start_id] + self.encode_remove(tokenizer, inputstr, args)+[tokenizer.end_id]
        srcids += [tokenizer.msg_id] + self.encode_remove(tokenizer, comment, args)
        srcids += [tokenizer.start_id] + self.encode_remove(tokenizer, newinputstr, args)+[tokenizer.end_id]
        tgtids = 'bad' if example['y'] == 1 else 'good'
        tgtids = self.encode_remove(tokenizer, tgtids, args)
        srcids, tgtids = self.pad_assert(srcids, tgtids, args, tokenizer)
        return RefineFeatures(example["id"], srcids, tgtids)
...
class SimpleElsDataset(ElsDataset):
    def tokenize(self, item):
        example, tokenizer, args = item
        diff = example["old_hunk"]
        difflines = diff.split("\n")[1:]        # remove start @@
...
class CodeDataset(Dataset):
    ...
    def tokenize(self, item):
        example, tokenizer, args = item
        oldlines = example["old_hunk"].split("\n")
        newlines = example["hunk"].split("\n")
        oldlines = [line[1:].strip() for line in oldlines]
        newlines = [line[1:].strip() for line in newlines]
        oldlines = "\n".join(oldlines)
        newlines = "\n".join(newlines)
        ...
        newlines = "<add>" + newlines.replace("\n", "<add>")
        comment = example["comment"]
        srcids = [tokenizer.start_id] + self.encode_remove(tokenizer, oldlines, args)+[tokenizer.end_id]
        # srcids += [tokenizer.msg_id] + self.encode_remove(tokenizer, comment, args)
        srcids += [tokenizer.start_id] + self.encode_remove(tokenizer, newlines, args)+[tokenizer.end_id]
        # srcids += [tokenizer.msg_id] + self.encode_remove(tokenizer, comment, args)
        tgtids = 'bad' if example['y'] == 1 else 'good'
        tgtids = self.encode_remove(tokenizer, tgtids, args)
        srcids, tgtids = self.pad_assert(srcids, tgtids, args, tokenizer)
...
class SimpleCodeDataset(CodeDataset):
    def tokenize(self, item):
        example, tokenizer, args = item
        oldlines = example["old_hunk"].split("\n")
        newlines = example["hunk"].split("\n")
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, pool, args, file_path, samplenum=-1):
        ...
        if os.path.exists(savep):
            ...
            logger.info("Reading examples from {}".format(file_path))
            examples = read_els_examples(file_path, samplenum, tokenizer)
            logger.info(f"Tokenize examples: {file_path}")
...
def read_els_examples(filename, data_num=-1, tokenizer=None):
    """Read examples from filename."""
    examples = []
    idx = 0
    with open(filename) as f:
        for line in f:
            ...
            example = ReviewExample(
                        ...
                        oldf=js["oldf"],
                        diff=js["hunk"],
                        msg=js["comment"] if "comment" in js else "",
                        cmtid=js["cmtid"] if "cmtid" in js else "",
                        max_len=maxl,
                        y=js["y"]
                    )
