...
nohup python -m torch.distributed.launch --nproc_per_node ${PER_NODE_GPU} --node_rank=${RANK} --nnodes=${NODES} --master_addr=${MASTER_HOST} --master_port=${MASTER_PORT} ../run_finetune_code_els.py  \
  ...
  --output_dir ./els_code_wocomment_model \
  --train_filename /data/hzh22/CodeReviewProcesser/postprocess/data/ref-train.jsonl \
  --dev_filename /data/hzh22/CodeReviewProcesser/postprocess/data/ref-valid.jsonl \
  ...
  --node_index=${RANK} \
  --seed 2233 \
  > train_reviewer_code_wocomment.log 2>&1 &
