...
from utils import CodeDataset, SimpleCodeDataset
...
def get_loader(data_file, args, tokenizer, pool, eval=False):
    ...
    if args.raw_input:
        dataset = SimpleCodeDataset(tokenizer, pool, args, data_file)
    else:
        dataset = CodeDataset(tokenizer, pool, args, data_file)
...
def eval_bleu_epoch(args, eval_dataloader, model, tokenizer):
    ...
    for i in range(len(golds)):
        pred_nls[i], golds[i] = CodeDataset.process_pred_gold(pred_nls[i], golds[i])
    ...
    for epoch in range(1, args.train_epochs + 1):
        ...
        for step, examples in enumerate(train_dataloader, 1):
            ...
            if global_step == args.train_steps and args.global_rank == 0:
                # end training
                # bleu = eval_bleu_epoch(args, valid_dataloader, model, tokenizer)
                output_dir = os.path.join(args.output_dir, "checkpoints-last" + "-" + "6.0")
            ...
            if args.global_rank == 0 and \
                ...
                # bleu = eval_bleu_epoch(args, valid_dataloader, model, tokenizer)
                output_dir = os.path.join(args.output_dir, "checkpoints-" + str(global_step) + "-" + "blue")
                save_model(model, optimizer, scheduler, output_dir, config)
                logger.info(
